...
static const char *const zip_errlist[33] = {
    ...
    "no permission\0",
    "out of memory\0",
    "invalid zip archive class_name\0",
    "make dir error\0",
    "symlink error\0",
                               ...
                               void *arg)
{
  ...
  for (i = 0; i < n; ++i)
  {
    ...
    {
      // Cannot normalize file class_name;
      err = ZIP_EINVENTNAME;
      goto out;
    }
                                ...
                                int *errnum)
{
  ...
  *errnum = 0;
  ...
  {
    // zip_t archive class_name is empty or NULL
                           ...
                           int case_sensitive)
{
  ...
  if (pzip->m_zip_mode == MZ_ZIP_MODE_READING)
  {
    ...
    if (!zip->entry.name)
    {
      // Cannot parse zip entry class_name
      return ZIP_EINVENTNAME;
    }
  ...
  /*
    ...
    4.4.17.1 The class_name of the file, with optional relative path.
    The path stored MUST not contain a drive or
    device letter, or a leading slash.  All slashes
    MUST be forward slashes '/' as opposed to
    backwards slashes '\' for compatibility with Amiga
    and UNIX file systems etc.  If input came from standard
    input, there is no file class_name field.
  */
  ...
  // ZIP64 header with NULL sizes (sizes will be in the data desc, just
  // after file data)
...
int zip_entry_openbyindex(struct zip_t *zip, size_t index)
{
  ...
  if (!zip->entry.name)
  {
    // local entry class_name is NULL
    return ZIP_EINVENTNAME;
  }
                ...
                int (*on_extract)(const char *filename, void *arg), void *arg)
{
  ...
  if (!zipname || !dir)
  {
    // Cannot parse zip archive class_name
    return ZIP_EINVZIPNAME;
  }
