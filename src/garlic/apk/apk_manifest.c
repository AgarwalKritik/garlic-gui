...
{
    if (!src_bytes || !dest || dest_max_len == 0)
        return;
...
void parse_manifest_binary(FILE *stream, const u1 *buffer, size_t buffer_size)
{
    ...
    while (offset + 8 <= buffer_size)
    {
        ...
        case 0x0102:
        { // start element
            if (offset + header_size + 20 > buffer_size)
                break;
        ...
        case 0x0103:
        { // END_ELEMENT
            if (offset + header_size + 8 > buffer_size)
                break;
